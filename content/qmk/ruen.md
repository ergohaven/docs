...
Для **macOS** доступна [сборка](https://github.com/VlaDi4eKK/qmk-hid-host/releases) от автора VlaDi4eKK (участник нашего чата), за что так же выражаем ему благодарность.

{{< callout icon="sparkles" >}}
  Со слов автора: "Это форк основного кода (практически без изменений), представляющий собой простую обертку на Swift для отображения иконки в трее и возможности обновлять конфиг на лету"
{{< /callout >}}

Приложение можно скачать как zip, закинуть его в **Программы** и запустить.

Основные функции:
- Управление автозапуском
- Управление конфигом, редактирование и обновление
- Детекция клавиатуры и пресеты для известных мне конфигов (в идеале добавить туда все популярные, но это уже не для меня затея)
- Возможность подцепить лаяуты в системе (например в макоси они не всегда обозначаются как ru/en)
- Конфиг автоматически формируется на основе доступных подключенных устройств

### Режим CAPS WORD

Режим CAPS WORD (включается на одноименную кнопку CAPS WORD) позволяет ввести одно слово в верхнем регистре и автоматически выйти из этого режима по нажатии на символ, разделяющий слова. В режиме RuEn поддержан режим CAPS WORD: символы, которые в английской раскладке являются разделителями слов (такие как `ЖЭХЪЁБЮ`) не прерывают режим CAPS WORD, если набираются в русской раскладке.
